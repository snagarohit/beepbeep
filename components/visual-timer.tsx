...
export const VisualTimer = React.forwardRef<HTMLDivElement, VisualTimerProps>(
  ({ isRotated, percentage, onTimeChange, onDrag }, ref) => {
    const lastMinuteRef = React.useRef<number | null>(null);
    
    const handleInteraction = (clientX: number, clientY: number, isDragging: boolean, target: HTMLElement) => {
    ...
    const onPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
      ...
      handleInteraction(e.clientX, e.clientY, false, e.currentTarget);
    };
    
    const onPointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
      if (e.buttons !== 1) return;
      handleInteraction(e.clientX, e.clientY, true, e.currentTarget);
    };
...
VisualTimer.displayName = "VisualTimer"; 